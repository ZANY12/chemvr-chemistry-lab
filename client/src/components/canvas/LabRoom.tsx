...
// Helper to generate coordinates for all 118 elements (group=x, period=y)
...
function PeriodicTableTexturePlane() {
  const texture = useMemo(() => {
    ...
    for (const el of elements) {
      const px = gridX + el.x * cellW;
      const py = gridY + el.y * cellH;
      ...
      ctx.fillText(String(el.n), px + 8, py + 6);

      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.font = 'bold 34px Arial';
      ctx.fillText(el.s, px + cellW / 2, py + cellH / 2 + 4);
    }
